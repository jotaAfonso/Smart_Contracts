...
data HelloBlockchainState =
    ...
    | SendReqState BuiltinByteString [(Integer,(POSIXTime,Slot))]
...
data HelloBlockchainInput =
    ...
    | SendReqInput BuiltinByteString [(Integer,(POSIXTime,Slot))] Value
...
transition State{stateData=oldData,stateValue} input = case (oldData, input) of
    ...
    (SendReqState _ _, SendRequestInput message triggerTimeStamps stateVal) -> Just(mempty, State{stateData = SendRespondState message triggerTimeStamps, stateValue = stateVal})
    (SendRespondState _ _, SendReqInput message triggerTimeStamps stateVal) -> Just(mempty, State{stateData = SendReqState message triggerTimeStamps, stateValue = stateVal})
...
transitionCheck state input context = case (state, input) of
    ...
    (SendReqState message triggerTimeStamps, SendRequestInput _ _ _) -> True
    (SendRespondState message triggerTimeStamps, SendReqInput _ _ _) -> True
...
validTransitions (SendReqState _ _) (SendRequestInput _ _ _) = True
validTransitions (SendRespondState _ _) (SendReqInput _ _ _) = True
...
getContractInfo = do
    ...
    case currState of
        ...
        Just (SendReqState _ triggerTimeStamps) -> pure $ (triggerTimeStamps)
...
getFields = do
    ...
    case currState of
        ...
        Just (SendReqState message triggerTimeStamps) -> pure $ (message, triggerTimeStamps)
...
sendReq :: Promise () HelloBlockchainSchema T.Text ()
sendReq = endpoint @"sendreq" $ \() -> do
    -- | Received the parameters
    currentState <- mapSMError' $ getCurrentStateSM client
    if isValidCallInState currentState (SendReqInput undefined undefined undefined) then do
    -- | Calling business logic function...
      oldVal <- fundsInContract
      (messageOld, triggerTimeStamps) <- getFields
      logic <- sendReqLogic messageOld triggerTimeStamps oldVal
      case logic of
        Left (message, stateVal, constraint) -> do
          res <- mapSMError' $ runContractStep client (SendReqInput message triggerTimeStamps stateVal) constraint
          case res of
                Just s -> do
                    logInfo ("Successful transaction to state: " <> show s)
                _ -> logError @String "Invalid operation in endpoint."
      ...
      logError @String "Invalid invocation of endpoint sendReq"
...
sendReqLogic :: BuiltinByteString ->
                [(Integer, (POSIXTime, Slot))] ->
                Value ->
                Contract ()
                         HelloBlockchainSchema
                         ...
                         (Either (BuiltinByteString,
                                  ...
                                  TxConstraints HelloBlockchainInput HelloBlockchainState)
                                 HelloBlockchainError)
sendReqLogic message triggerTimeStamps stateVal = do returnOutputOk
                 where output :: LogicOutput
                       output = LogicOutput message stateVal mempty
                       printMsg :: String -> Contract () HelloBlockchainSchema T.Text ()
                       printMsg msg = logInfo @String msg
...
contract = selectList[init, sendRespond, sendReq, sendRequest]
